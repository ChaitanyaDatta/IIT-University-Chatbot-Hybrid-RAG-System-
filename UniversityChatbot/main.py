...
def _load_vocab() -> dict:
    for p in [
        project_root / "utilities" / "config" / "vocabulary.json",
        project_root / "vocabulary.json",
    ]:
        if p.exists():
            with open(p) as f:
                return json.load(f)
...
_NON_NAME_WORDS: set = set(_VOCAB.get("non_name_words", []))
_DIRECT_NON_CONTACT_ANCHORS: set = set(_VOCAB.get("non_contact_anchors", []))
